    ...
    <section id="map" className="py-24 px-6 relative overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-bold mb-6 text-marine-deep">Nos Routes Maritimes</h2>
        ...
        <div className="relative aspect-video w-full bg-marine-deep rounded-[3rem] border border-marine-ocean/20 shadow-2xl overflow-hidden group">
          {/* Mock Map Background */}
          <div className="absolute inset-0 bg-marine-deep">
            {/* Decorative Grid */}
            ...
            <svg className="absolute inset-0 w-full h-full" viewBox="0 0 1000 600">
              ...
              {circuits.map((circuit, idx) => {
                const colors = ['#3b82f6', '#0ea5e9', '#1e3a8a'];
          ...
          <div className="absolute top-8 left-8 flex flex-col gap-4">
            {circuits.map((circuit, idx) => (
              <div key={circuit.id} className="bg-white/10 backdrop-blur-md px-4 py-2 rounded-xl flex items-center gap-3 border border-white/10 hover:bg-white/20 cursor-pointer transition-all">
                <div className={`w-3 h-3 rounded-full ${idx === 0 ? 'bg-blue-500' : idx === 1 ? 'bg-sky-400' : 'bg-blue-800'}`}></div>
          ...
          <div className="absolute bottom-8 right-8 bg-white/10 backdrop-blur-xl p-4 rounded-2xl border border-white/10 max-w-xs">
            <div className="flex items-center gap-3 mb-2">
              <Compass className="text-marine-turquoise animate-spin-slow" />
              <h4 className="font-bold text-white">Navigation Interactive</h4>
            </div>
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-500">
            <div className="bg-white/10 backdrop-blur-md p-6 rounded-full border border-white/20">
              <MapIcon size={48} className="text-marine-turquoise animate-bounce" />
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Background Elements */}
      <div className="absolute -bottom-24 -left-24 w-96 h-96 bg-marine-ocean/10 blur-[150px] rounded-full"></div>
    </section>
  );
}
